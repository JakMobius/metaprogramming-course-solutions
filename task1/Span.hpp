...
template <typename T, size_t extent> class StaticSpan {
public:
...
template <typename T> class DynamicSpan {
public:
  using element_type = T;
  using size_type = size_t;

  template <typename Container> DynamicSpan(Container &container) {
    this->data_ = container.data();
    this->extent_ = container.size();
  }

  template <class Iter> DynamicSpan(Iter begin, Iter end) {
    this->data_ = &*begin;
    this->extent_ = end - begin;
  }

  template <class Iter> DynamicSpan(Iter begin, size_t size) {
    this->data_ = &*begin;
    this->extent_ = size;
  }

  constexpr size_type Size() const { return extent_; }
...
template <typename T, size_t extent = std::dynamic_extent> class Span;

template <class Base> class SpanBase : public Base {
public:
  ...
  iterator end() const { return this->data_ + this->Size(); }
  ...
  pointer Data() const { return this->data_; }

  reference operator[](int index) const { return *(begin() + index); };

  Span<element_type> Last(int length) const {
    return Span<element_type>(this->end() - length, this->end());
  }

  template <int length> constexpr Span<element_type, length> Last() const {
    return Span<element_type, length>(this->Data() + this->Size() - length);
  }

  Span<element_type> First(int length) const {
    return Span<element_type, std::dynamic_extent>(this->Data(),
                                                   this->Data() + length);
  }

  template <int length> constexpr Span<element_type, length> First() const {
...
template <typename Iter> Span(Iter, Iter) -> Span<typename Iter::value_type>;
